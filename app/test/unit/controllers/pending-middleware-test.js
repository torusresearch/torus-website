...
const { createPendingNonceMiddleware, createPendingTxMiddleware } = require('../../../src/utils/createMetamaskMiddleware')
const { txMetaStub } = require('../lib/network-stub')
...
describe('#createPendingNonceMiddleware', () => {
  ...
  it('should call next if not a eth_getTransactionCount request', done => {
    const request = { method: 'eth_getBlockByNumber' }
    const res = {}
    pendingNonceMiddleware(request, res, () => done())
  })
  it('should call next if not a "pending" block request', done => {
    const request = { method: 'eth_getTransactionCount', params: [address] }
    const res = {}
    pendingNonceMiddleware(request, res, () => done())
  })
  it('should fill the result with a the "pending" nonce', done => {
    const request = { method: 'eth_getTransactionCount', params: [address, 'pending'] }
    const res = {}
...
describe('#createPendingTxMiddleware', () => {
  ...
  it('should call next if not a eth_getTransactionByHash request', done => {
    const request = { method: 'eth_getBlockByNumber' }
    const res = {}
    pendingTxMiddleware(request, res, () => done())
  })

  it('should call next if no pending txMeta is in history', done => {
    const request = { method: 'eth_getTransactionByHash', params: [address] }
    const res = {}
    pendingTxMiddleware(request, res, () => done())
  })
  ...
  it('should fill the result with a the "pending" tx the result should match the rpc spec', done => {
    ...
    const request = { method: 'eth_getTransactionByHash', params: [address, 'pending'] }
    const res = {}
    pendingTxMiddleware(
      request,
      res,
