...
import { arrayify, hexlify, keccak256, toUtf8Bytes, toUtf8String } from 'ethers'
  ...
  return {
    ...
    set: async (pairs, shouldBackup) => {
      for (const pair of pairs) {
        localStorage.setItem(`${ConnextClientStorePrefix}:${pair.path}`, typeof pair.value === 'string' ? pair.value : JSON.stringify(pair.value))

        if (shouldBackup && pisaClient && pair.path.match(/\/xpub.*\/channel\/0x[0-9a-fA-F]{40}/) && pair.value.freeBalanceAppInstance) {
          try {
    ...
    restore: async () => {
      return pisaClient
        ? (await pisaClient.restore(digest => wallet.signMessage(arrayify(digest)), wallet.address, await wallet.provider.getBlockNumber())).map(b =>
            JSON.parse(toUtf8String(arrayify(b.data)))
          )
        : []
    }
  }
