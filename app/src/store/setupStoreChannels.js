...
if (!isMain) {
  ...
  scwStream.on('data', async chunk => {
    if (chunk.name === 'create_scw_request') {
      const response = await VuexStore.dispatch('createSmartContractWallet')
      log.info(response)
  ...
  getScwStream.on('data', async chunk => {
    if (chunk.name === 'scw_address_request') {
      const response = await VuexStore.dispatch('getSmartContractWalletAddress')
