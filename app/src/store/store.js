...
import { patch, post } from '../utils/httpHelpers'
    ...
    for (const id in txs) {
      ...
      if (txMeta.status === 'submitted' && id >= 0) {
        ...
        if (state.pastTransactions.findIndex(x => x.transaction_hash === txObject.transaction_hash && x.network === txObject.network) === -1) {
          ...
          const apiMethod = relayer && !txObject.transaction_hash.includes('PENDING_') ? 'patch' : 'post'
          log.info('transactionAPI', transactionsAPI, apiMethod, txObject)

          if (apiMethod === 'patch') {
            const oldTxHash = 'PENDING_'.concat(txMeta.id)
            patch(
              `${config.api}/transactionSCW/txHash`,
              { newTxHash: txObject.transaction_hash, oldTxHash },
              {
                headers: {
              ...
              .catch(error => log.error(error, 'unable to update transaction'))
          } else {
            post(transactionsAPI, txObject, {
              headers: {
                Authorization: `Bearer ${state.jwtToken}`,
                'Content-Type': 'application/json; charset=utf-8'
              }
