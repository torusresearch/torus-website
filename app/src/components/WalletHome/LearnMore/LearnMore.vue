  ...
  <v-stepper v-model="e1" class="learn-more">
    <v-stepper-items>
        ...
        <v-img :src="require(`../../../../public/images/learn-more-intro.svg`)">
          <v-layout>
            <v-flex xs12 class="mt-12 ml-12">
              <div class="display-1 torusBrand1--text text--darken-4 font-weight-bold mb-2">{{ t('walletHome.welcome') }}</div>
              <img width="150" :src="require(`../../../../public/images/torus-logo-blue.svg`)" />
            </v-flex>
          </v-layout>
        </v-img>
        <v-btn id="get-started-btn" depressed large color="white" class="torusBrand1--text px-12 next-btn" @click="e1 = 2">Next</v-btn>
      </v-stepper-content>
      <v-stepper-content v-for="content in contents" :key="content.title" :step="content.step">
        ...
        <v-btn id="next-step-btn" depressed large class="torusBrand1 px-12 next-btn" @click="nextStep(content.step)">
