<template>
  <v-app-bar app flat class="header-container" :color="$vuetify.theme.dark ? '' : 'white'">
    <div v-if="!showNav" class="d-flex align-end">
      <img v-if="!$vuetify.breakpoint.xsOnly" class="home-link mr-1" alt="Torus Logo" height="30" :src="getLogo.logo" />
      <img v-else src="../../../assets/img/icons/binance-chain-logo.svg" height="30" alt="Torus Logo" />
    </div>
    <div v-else class="d-flex align-end">
      <router-link v-if="!$vuetify.breakpoint.xsOnly" :to="{ name: 'walletHome' }">
        <img class="home-link mr-1" alt="Torus Logo" height="30" :src="getLogo.logo" />
      </router-link>
