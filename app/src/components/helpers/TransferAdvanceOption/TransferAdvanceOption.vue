  ...
  <v-dialog v-model="dialog" persistent>
    <template v-slot:activator="{ on }">
      <a id="advance-option-link" class="float-right primary--text subtitle-2" v-show="displayAmount" v-on="on">
        {{ $vuetify.lang.t('$vuetify.walletTransfer.advancedOptions') }}
      </a>
    </template>
              ...
              <v-layout wrap>
                <v-flex xs12 sm6 px-4>
                  <span class="subtitle-2">
                    {{ $vuetify.lang.t('$vuetify.walletTransfer.gasPrice') }} (GWEI)
                    <HelpTooltip title="Gas Price">
                      <template v-slot:description>
                        <div class="body-2 text-justify">
                          <span class="font-weight-medium">{{ $vuetify.lang.t('$vuetify.walletTransfer.gasPriceDesc1') }}</span>
                          {{ $vuetify.lang.t('$vuetify.walletTransfer.gasPriceDesc2') }}
                          <span class="font-weight-medium">{{ $vuetify.lang.t('$vuetify.walletTransfer.gasPriceDesc3') }}</span>
                          {{ $vuetify.lang.t('$vuetify.walletTransfer.gasPriceDesc4') }}
                        </div>
                        <div class="caption mt-1">
                          1 Gwei=10
                          <sup>-9</sup>
                          ETH
                          <small>({{ $vuetify.lang.t('$vuetify.walletTransfer.gasPriceDesc5') }})</small>
                        </div>
                      </template>
                    </HelpTooltip>
                  </span>
                  <v-text-field
                    id="gas-price"
                    :placeholder="$vuetify.lang.t('$vuetify.walletTransfer.enterValue')"
                    outlined
                ...
                <v-flex xs12 sm6 px-4>
                  <span class="subtitle-2">
                    {{ $vuetify.lang.t('$vuetify.walletTransfer.gasLimit') }}
                    <HelpTooltip
                      :title="$vuetify.lang.t('$vuetify.walletTransfer.gasLimit')"
                      :description="$vuetify.lang.t('$vuetify.walletTransfer.gasLimitDesc')"
                    ></HelpTooltip>
                  </span>
                  <v-text-field id="advanced-gas" readonly outlined :value="advancedGas" required type="number"></v-text-field>
                </v-flex>
                <v-flex xs12 sm6 px-4>
                  <span class="subtitle-2">{{ $vuetify.lang.t('$vuetify.walletTransfer.sendAmount') }}</span>
                ...
                <v-flex xs12 sm6 px-4>
                  <span class="subtitle-2">{{ $vuetify.lang.t('$vuetify.walletTransfer.transferFee') }}</span>
                  <template v-if="$vuetify.breakpoint.xsOnly">
                ...
                <v-flex xs12 sm6 px-4 :class="$vuetify.breakpoint.xsOnly ? 'mt-5' : ''">
                  <span class="subtitle-2">{{ $vuetify.lang.t('$vuetify.walletTransfer.newTotal') }}</span>
                  <template v-if="$vuetify.breakpoint.xsOnly">
