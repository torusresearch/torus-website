<template>
  ...
  >
    <v-carousel-item
      ...
      delimiter-icon="$vuetify.icons.minus"
      :show-arrows="false"
    >
      <v-layout align-center fill-height px-10>
        <v-flex class="text-center">
          ...
          <div class="text-h6 text-sm-h5 font-weight-medium mb-3 text_2--text px-2">{{ slide.title }}</div>
          <div class="text-caption text-sm-body-1 text_2--text">{{ slide.subtitle1 }}</div>
          <div v-if="slide.subtitle2" class="text-caption text-sm-body-1 text_2--text">{{ slide.subtitle2 }}</div>
          <!-- <v-btn
...
export default {
  ...
  computed: {
    slides() {
      const slides = [
        {
          image: `login-bg-new-${this.$vuetify.theme.dark ? 'dark-' : ''}1.svg`,
          title: this.t('login.slide1Title'),
          subtitle1: this.t('login.slide1Subtitle1'),
          subtitle2: this.t('login.slide1Subtitle2'),
        },
        {
          image: `login-bg-new-${this.$vuetify.theme.dark ? 'dark-' : ''}2.svg`,
          title: this.t('login.slide2Title'),
          subtitle1: this.t('login.slide2Subtitle1'),
          subtitle2: this.t('login.slide2Subtitle2'),
        },
        {
